...
class SizingAgent:
    ...
    def invoke(self, constraints: dict, history: list, device_names: list, topology: dict = None, specs: dict = None, itail: float = None, idc_max: float = None, topology_config: dict = None) -> dict:
        ...
        # 若阶段二未返回完整器件表，从阶段一 incremental_calculation 兜底（支持嵌套 {"xm8":{L,gm_id}} 或平键 xm8_L）
        inc = (stage1_json.get("incremental_calculation") or {}) if isinstance(stage1_json, dict) else {}
        inc_lower = {str(k).lower(): v for k, v in inc.items()} if isinstance(inc, dict) else {}
        for d in device_names:
            if d in sizing_dict and isinstance(sizing_dict.get(d), dict):
                continue
            d_low = str(d).lower()
            L_val, g_val = None, None
            if d_low in inc_lower and isinstance(inc_lower[d_low], dict):
                L_val = inc_lower[d_low].get("L")
                g_val = inc_lower[d_low].get("gm_id")
            if L_val is None:
        ...
        # 仅对「本轮推理显式修改」的管子用 sizing_dict；其余沿用上一轮 params，避免 format 用约束中点把未改管带偏导致 W 爆炸
        explicit_set = {d for d in device_names if str(d).lower() in inc_lower}
        base_entry = last_entry
        if len(history) >= 2 and (_is_collapse(last_entry, w_max_um=w_collapse) or collapse_by_history):
            base_entry = history[-2]
        base_params = base_entry.get("params") or {}
        sizing_lower = {str(k).lower(): k for k in sizing_dict.keys()}
        out = {"think": think_for_log}
        for d in device_names:
            key = d if d in sizing_dict else sizing_lower.get(str(d).lower())
            from_sizing = key and isinstance(sizing_dict.get(key), dict)
            if d in explicit_set and from_sizing:
                out[d] = {k: sizing_dict[key][k] for k in ("L", "gm_id") if k in sizing_dict[key]}
            elif d not in explicit_set:
                prev = base_params.get(d) or base_params.get(str(d).upper()) or base_params.get(str(d).lower())
                if isinstance(prev, dict) and (prev.get("L") is not None or prev.get("gm_id") is not None):
                    out[d] = {"L": float(prev.get("L", 0.5)), "gm_id": float(prev.get("gm_id", 10))}
            if d not in out:
                if from_sizing:
                    out[d] = {k: sizing_dict[key][k] for k in ("L", "gm_id") if k in sizing_dict[key]}
                else:
